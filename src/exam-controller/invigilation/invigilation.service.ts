...
export class InvigilationService {
      ...
      const returnObj = roomswithPendingInvigilator.map((room) => {
        ...
        if (room.invigilator1_id) {
          temp['invigilator1'] = {
            ...
            scan_date: room.invigilator1_assign_time.toLocaleDateString(
              'en-IN',
              {
                timeZone: 'Asia/Kolkata',
              },
            ),
            scan_time: room.invigilator1_assign_time.toLocaleTimeString(
              'en-IN',
              {
                timeZone: 'Asia/Kolkata',
              },
        ...
        if (room.invigilator2_id) {
          temp['invigilator2'] = {
            ...
            scan_date: room.invigilator2_assign_time.toLocaleDateString(
              'en-IN',
              {
                timeZone: 'Asia/Kolkata',
              },
            ),
            scan_time: room.invigilator2_assign_time.toLocaleTimeString(
              'en-IN',
              {
                timeZone: 'Asia/Kolkata',
              },
        ...
        if (room.invigilator3_id) {
          temp['invigilator3'] = {
            ...
            scan_date: room.invigilator3_assign_time.toLocaleDateString(
              'en-IN',
              {
                timeZone: 'Asia/Kolkata',
              },
            ),
            scan_time: room.invigilator3_assign_time.toLocaleTimeString(
              'en-IN',
              {
                timeZone: 'Asia/Kolkata',
              },
  ...
  async editStudentEligibility(body: EditStudentEligibilityDto) {
    try {
      ...
      const stIdx = roomDoc.students.findIndex(
        (st) => st.sap_id === body.sap_id,
      );
      if (stIdx === -1) {
  ...
  async dutySheetUpload(body: DutySheetUploadDto) {
    try {
      ...
      if (
        body.inv_sap_ids.filter((id) => body.fly_sap_ids.includes(id)).length
      ) {
        throw new HttpException(
